...
for nn=1:Nsteps
    ...
    zetan(:,1) = zetan(:,2); % ??? -> WRITE THE zeta BC AT GHOST zeta NODE j=1
    % Boundary condition at j=N (real node)
    zetan(:,N) = zetan(:,N-1); % ??? -> WRITE THE zeta BC AT REAL zeta NODE j=N
    ...
    for k=1:2
        if multipas(k) == 1 %This is evaluated first if nn odd
            ...
            % Boundary condition at j=N (real node)
            un(:,N) = un(:,N-1); % ??? -> WRITE THE u BC AT REAL u NODE j=N
